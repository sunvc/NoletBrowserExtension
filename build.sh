...
if [ -f ".output/nolet-$PACKAGE_VERSION-chrome.zip" ]; then
    echo "✅ Chrome 扩展构建成功"
...
if [ -f ".output/nolet-$PACKAGE_VERSION-firefox.zip" ]; then
    echo "✅ Firefox 扩展构建成功"
...
# 检查 Edge 扩展
EDGE_SUCCESS=false
if [ -f ".output/nolet-$PACKAGE_VERSION-edge.zip" ]; then
    echo "✅ Edge 扩展构建成功"
...
if [ "$CHROME_SUCCESS" = true ] || [ "$FIREFOX_SUCCESS" = true ] || [ "$EDGE_SUCCESS" = true ]; then
    ...
    if [ "$CHROME_SUCCESS" = true ]; then
        echo "   📦 Chrome: .output/nolet-$PACKAGE_VERSION-chrome.zip"
        echo "   📏 文件大小 / File size:"
        ls -lh .output/nolet-$PACKAGE_VERSION-chrome.zip
    fi
    
    if [ "$FIREFOX_SUCCESS" = true ]; then
        echo "   📦 Firefox: .output/nolet-$PACKAGE_VERSION-firefox.zip"
        echo "   📏 文件大小 / File size:"
        ls -lh .output/nolet-$PACKAGE_VERSION-firefox.zip
    fi
