...
const IsolatedContainer: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  return (
    <Box
...
const StatusChip: React.FC<{ record: HistoryRecord }> = ({ record }) => {
  ...
  if (responseCode === -1) {
    ...
    const errorMessage = isHTTPError
      ? responseMessage.replace("HTTP error! status: ", "")
      : responseMessage;
...
export default function RecordDetailModal({
  ...
  useEffect(() => {
    ...
    // 构建 parameters
    const parameters = (record.parameters ?? []).reduce(
      (acc, { key, value }) => {
    ...
    try {
      sendTimestamp = dayjs
        .tz(record.createdAt, "YYYY-MM-DD HH:mm:ss", timezone)
        .unix();
    } catch (error) {
    ...
    const displayParameters = Object.fromEntries(
      Object.entries(parameters).filter(
        ([key, value]) =>
          !["timezone", "sendTimestamp"].includes(key) && value !== ""
      )
    );
    ...
    if (displayParameters.device_keys) {
      const deviceKeysArray = (
        Array.isArray(displayParameters.device_keys)
          ? displayParameters.device_keys
          : String(displayParameters.device_keys)
              .split(",")
              .map((k: string) => k.trim())
      ).filter(Boolean);
  ...
  const bodyRenderer = () => {
    if (record?.inspectType === "image" && record.url) {
      // inspectType 为 image 时
      return (
        <Stack direction="row" gap={1} alignItems="center">
          <Box
            sx={{
    ...
    return (
      // inspectType 可能会有 omnibox 等其他类型
      <Stack direction="row" gap={1} alignItems="center">
        <Typography
          variant="body2"
          sx={{
          ...
          >
            ...
            >
              ...
              >
                ...
                <Stack
                  alignItems="center"
                  sx={{ flex: 1, fontWeight: 600, fontSize: "1.0625rem" }}
                >
                  {/* 推送详情 */}
              ...
              <Box sx={{ px: 3, py: 1.5, pb: 1 }}>
                ...
                >
                  ...
                  <PreviewCard parameters={parameters} />
                </Stack>
              </Box>
              ...
              <Box ref={contentRef} sx={{ flex: 1, overflow: "auto", p: 3 }}>
                <IsolatedContainer>
                  <Stack spacing={1} sx={{ pb: 8 }}>
                    ...
                    {/* 响应信息 */}
                    {record.responseJson.message.startsWith("utils.api.") ===
                      false && (
                      <DetailRow
                        icon={<LinkIcon fontSize="small" />}
