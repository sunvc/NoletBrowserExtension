...
export default function SendPush({
  ...
  // 从本地存储恢复消息内容
  useEffect(() => {
    const savedMessage = localStorage.getItem("nolet-draft-message");
    if (savedMessage) {
  ...
  const handleMessageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // 暂存消息内容
    if (newMessage.trim()) {
      localStorage.setItem("nolet-draft-message", newMessage);
    } else {
      localStorage.removeItem("nolet-draft-message");
    }
  };
  ...
  const handleShortcutTriggered = async () => {
    ...
    try {
      ...
      setResult({
        type: "error",
        message: t("push.errors.clipboard_failed", {
  ...
  const handleShortcutSend = async () => {
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        setResult({
          type: "error",
          message: t("push.errors.send_failed", { message: response.message }),
        });
      }
    } catch (error) {
      /* 发送失败: {{message}} */
      const errorMessage =
        error instanceof Error ? error.message : t("common.error_unknown"); // 未知错误
  ...
  const handleRecall = async () => {
    ...
    try {
      ...
      if (!historyRecord) {
        setResult({
          type: "error",
          message: t("push.recall.record_not_found"),
        });
        return;
      }

      const recallUrl = `${historyRecord.apiUrl}?id=${encodeURIComponent(
        lastPushUuid
      )}&delete=1`;
  ...
  const handleSendClipboard = async () => {
    ...
    try {
      ...
      if (response.code === 200) {
        ...
        localStorage.setItem("nolet-draft-message", clipboardText.trim()); // 更新暂存内容为剪切板内容
      } else {
        /* 发送失败: {{message}} */
        const errorMessage = response.message || t("common.error_unknown");
        const finalMessage = errorMessage.startsWith("utils.api.")
  ...
  const handleAddDevice = async (
    ...
    try {
      ...
      setResult({
        type: "error",
        message: t("push.errors.add_device", {
          message: error instanceof Error ? error.message : "未知错误",
        }),
      });
    }
    ...
    if (shouldAutoAddDevice) {
      ...
      const loadContentToInput = async () => {
        try {
          ...
          const omniboxResult = await browser.storage.local.get(
            "nolet_omnibox_message"
          );
          if (
            omniboxResult.nolet_omnibox_message &&
            omniboxResult.nolet_omnibox_message.trim()
          ) {
          ...
          if (clipboardText && clipboardText.trim()) {
            setMessage(clipboardText.trim());
            // 更新本地存储
            localStorage.setItem("nolet-draft-message", clipboardText.trim());
          }
        } catch (error) {
  ...
  const [alertToShow, setAlertToShow] = React.useState<{
    type: "success" | "error";
    message: string;
  } | null>(null);
                          ...
                          <Stack direction="row" gap={1}>
                            ...
                            {selectedDevices.length > 1 ? (
                              <Tooltip
                                title={
                                  recallLoading
                                    ? t("push.recall.loading")
                                    : t("push.recall.button2")
                                }
                              >
                                <IconButton
                                  size="small"
            ...
            >
              <ShortcutTips
                tips={[
                  ...
                  {
                    description: t("push.open_extension"),
          ...
          <Stack spacing={2}>
            ...
            <Alert severity="info" icon={<KeyboardIcon />}>
              <Typography variant="body2">
                {/* 按 Enter 发送，按 Esc 取消 */}
                {t("push.shortcut_enter_prefix")} <strong>Enter</strong>{" "}
                {t("push.shortcut_esc_prefix")} <strong>Esc</strong>{" "}
                {t("push.shortcut_esc_suffix")}
              </Typography>
            </Alert>
          </Stack>
        </DialogContent>
