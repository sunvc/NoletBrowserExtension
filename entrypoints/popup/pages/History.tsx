...
const RecordDetailModal = React.lazy(
  () =>
    // 懒加载RecordDetailModal组件
    import(
...
export default function History() {
  ...
  const [scrollToRecordId, setScrollToRecordId] = useState<number | undefined>(
    undefined
  );
      ...
      for (const record of historyToImport) {
        try {
          ...
          const exists = existingRecords.some(
            (existing) => existing.uuid === record.uuid
          );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setAlert({
        type: "success",
        message: t("history.messages.delete_success", {
  ...
  const handleExport = async () => {
    try {
      ...
      setAlert({
        type: "success",
        message: t("history.messages.export_success"),
      });
    } catch (error) {
  ...
  const handleExportRecords = async (records: HistoryRecord[]) => {
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `nolet-history-selected-${
        new Date().toISOString().split("T")[0]
      }.json`;
      document.body.appendChild(a);
      a.click();
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setAlert({
        type: "success",
        message: t("history.messages.import_success_message", {
          success: result.success,
          skipped: result.skipped,
          errors: result.errors,
        }), // 导入完成：成功 {{success}} 条，跳过 {{skipped}} 条，失败 {{errors}} 条
      });
    } catch (error) {
      console.error("导入失败:", error);
      setAlert({
        type: "error",
        message: t("history.messages.import_failed", {
          message:
