  ...
  try {
    ...
    if (advancedParams) {
      ...
      processedAdvancedParams = Object.fromEntries(
        Object.entries(advancedParams).filter(
    ...
    const pushParams: PushParams = {
      ...
      message,
      devices: devices ? devices : [device], // 设备信息, 可能有多个设备 (用于API)
      ...
      ...(processedAdvancedParams &&
        Object.fromEntries(
          Object.entries(processedAdvancedParams).filter(
            ([key]) => !["title", "url"].includes(key)
          )
        )),
    };
    ...
    // 记录推送历史
    await recordPushHistory(message, device.apiURL, device.alias, response, {
    ...
    if (device) {
      const errorResponse = {
        ...
        message:
          error instanceof Error ? error.message : "common.error_unknown",
        timestamp: Date.now(),
      };
...
export function validateApiURL(url: string): boolean {
  ...
  const apiUrlPattern =
    /^(https?:\/\/)?([a-zA-Z0-9.-]+(\.[a-zA-Z]{2,})?|localhost|(\d{1,3}\.){3}\d{1,3})(:\d+)?\/[a-zA-Z0-9_-]+\/?.*$/;
